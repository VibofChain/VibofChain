...
#include "vibofchaingui.h"

#include "vibofchainunits.h"
...
const std::string VibofChainGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString VibofChainGUI::DEFAULT_WALLET = "~Default";

double GetPoSKernelPS();

VibofChainGUI::VibofChainGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...

{
    ...
    // MacDockIconHandler::instance()->setIcon(QIcon(":icons/about_vbc"));
#endif
...
void VibofChainGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to an VibofChain address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and vibofchain: URIs)"));
    ...
    transferTokenAction->setStatusTip(tr("Transfer tokens to VBC addresses"));
    ...
    aboutAction = new QAction(platformStyle->TextColorIcon(":/icons/about_vbc"), tr("&About %1").arg(tr(PACKAGE_NAME)), this);
    ...
    toggleHideAction = new QAction(platformStyle->TextColorIcon(":/icons/about_vbc"), tr("&Show / Hide"), this);
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your VibofChain addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified VibofChain addresses"));
    ...
    openAction->setStatusTip(tr("Open a vibofchain: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible VibofChain command-line options").arg(tr(PACKAGE_NAME)));
...
void VibofChainGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void VibofChainGUI::createToolBars()
{
...
void VibofChainGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool VibofChainGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool VibofChainGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void VibofChainGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void VibofChainGUI::setWalletActionsEnabled(bool enabled)
{
...
void VibofChainGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void VibofChainGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void VibofChainGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void VibofChainGUI::optionsClicked()
{
...
void VibofChainGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void VibofChainGUI::showDebugWindow()
{
...
void VibofChainGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void VibofChainGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void VibofChainGUI::openClicked()
{
...
void VibofChainGUI::gotoOverviewPage()
{
    footerWidget->show();
    overviewAction->setChecked(true);
...
void VibofChainGUI::gotoHistoryPage()
{
    footerWidget->hide();
...
void VibofChainGUI::gotoReceiveCoinsPage()
{
    footerWidget->hide();
...
void VibofChainGUI::gotoSendCoinsPage(QString addr)
{
    footerWidget->hide();
...
void VibofChainGUI::gotoSignMessageTab(QString addr)
{
    footerWidget->hide();
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void VibofChainGUI::gotoVerifyMessageTab(QString addr)
{
    footerWidget->hide();
...
/** TOKENS START */
void VibofChainGUI::gotoTokensPage()
{
    footerWidget->hide();
...
void VibofChainGUI::gotoCreateTokensPage()
{
    footerWidget->hide();
...
void VibofChainGUI::gotoManageTokensPage()
{
    footerWidget->hide();
...
void VibofChainGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to VibofChain network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void VibofChainGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void VibofChainGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void VibofChainGUI::updateHeadersSyncProgressLabel()
{
...
void VibofChainGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void VibofChainGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("VibofChain"); // default title
...
void VibofChainGUI::changeEvent(QEvent *e)
{
...
void VibofChainGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void VibofChainGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void VibofChainGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& tokenName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (tokenName == "VBC")
        msg += tr("Amount: %1\n").arg(VibofChainUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(VibofChainUnits::formatWithCustomName(tokenName, amount, MAX_TOKEN_UNITS, true));
...
void VibofChainGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void VibofChainGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool VibofChainGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool VibofChainGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void VibofChainGUI::setHDStatus(int hdEnabled)
{
...
void VibofChainGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void VibofChainGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void VibofChainGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void VibofChainGUI::updateStakingIcon()
{
    if (walletFrame) {
...
void VibofChainGUI::detectShutdown()
{
...
void VibofChainGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void VibofChainGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void VibofChainGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(VibofChainGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void VibofChainGUI::subscribeToCoreSignals()
{
...
void VibofChainGUI::unsubscribeFromCoreSignals()
{
...
void VibofChainGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void VibofChainGUI::toggleStakingActive()
{
    ...
    menu(0)
{
    ...
    QList<VibofChainUnits::Unit> units = VibofChainUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const VibofChainUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(VibofChainUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (VibofChainUnits::Unit u : VibofChainUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(VibofChainUnits::name(u)), this);
...
{
    setText(VibofChainUnits::name(newUnits));
}
