...
{
    // Call into TxToUniv() in vibofchain-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in vibofchain-common, so we query them here and push the
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    ...
                    if (spentInfo.addressType == 1) {
                        in.pushKV("address", CVibofChainAddress(CKeyID(spentInfo.addressHash)).ToString());
                    } else if (spentInfo.addressType == 2) {
                        in.pushKV("address", CVibofChainAddress(CScriptID(spentInfo.addressHash)).ToString());
                    }
                }
                newVin.push_back(in);
            }
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "Outputs are addresses (paired with a VBC amount, data or object specifying an token operation) or data.\n"
            "Returns hex-encoded raw transaction.\n"
            "Note that the transaction's inputs are not signed, and\n"
            "it is not stored in the wallet or transmitted to the network.\n"

            "\nPaying for Token Operations:\n"
            "  Some operations require an amount of VBC to be sent to a burn address:\n"
            "    transfer:       0\n"
            ...
            "     {\n"
            "       \"address\":                          (string, required) The destination vibofchain address.  Each output must have a different address.\n"
            "         x.xxx                             (numeric or string, required) The VBC amount\n"
            "           or\n"
...
//            "                                        If provided, it is an error if explicit sequence numbers are incompatible.\n"
            ...
            + HelpExampleCli("createrawtransaction", "\"[{\\\"txid\\\":\\\"mycoin\\\",\\\"vout\\\":0}]\" \"{\\\"RXissueUniqueTokenXXXXXXXXXXWEAe58\\\":20,\\\"change_address\\\":change_amount,\\\"issuer_address\\\":{\\\"issue_unique\\\":{\\\"root_name\\\":\\\"MYTOKEN\\\",\\\"token_tags\\\":[\\\"VBCHA\\\",\\\"BETA\\\"],\\\"]}}}\"")
    ...
    for (const std::string& name_ : addrList) {
        if (name_ == "data") {
            ...
            if (!IsValidDestination(destination)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid VibofChain address: ") + name_);
            }
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) vibofchain address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
