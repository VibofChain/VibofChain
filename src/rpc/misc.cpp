...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given vibofchain address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The vibofchain address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The vibofchain address validated\n"
    ...
    if (isValid)
    {
        ...
        CVibofChainAddress address(request.params[0].get_str());
        if (address.IsOfflineStakingAddress(Params())) {
            CVibofChainAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CVibofChainAddress spendingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: VibofChain address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are vibofchain addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) vibofchain address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The vibofchain address to use for the signature.\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "vibofchain-cli and the GUI. There is no server-side difference."
        );
...
{
    if (type == 2) {
        address = CVibofChainAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CVibofChainAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
static bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        CVibofChainAddress dest(address);
        uint160 hashBytes;
        int type = 0;
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  {\n"
            "    \"address\"  (string) The base58check encoded address\n"
            "    \"tokenName\"  (string) The name of the associated token (VBC for VibofChain)\n"
    ...
    if (includeTokens) {
        ...
        if (!mempool.getAddressIndex(addresses, VBC, indexes)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
        }
    }
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"tokenName\"   (string, optional) Get UTXOs for a particular token instead of VBC ('*' for all tokens).\n"
            "}\n"
            "\nResult\n"
            "[\n"
            "  {\n"
            "    \"address\"  (string) The address base58check encoded\n"
            "    \"tokenName\" (string) The token associated with the UTXOs (VBC for VibofChain)\n"
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++) {
        ...
        int64_t timeLock = (tokenName == "VBC") ? it->first.timeLock : nTokenLockTime;
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1 || !request.params[0].isObject())
        throw std::runtime_error(
            ...
            "  \"tokenName\"   (string, optional) Get deltas for a particular token instead of VBC.\n"
            "}\n"
            "\nResult:\n"
            "[\n"
            "  {\n"
            "    \"tokenName\"  (string) The token associated with the deltas (VBC for VibofChain)\n"
            "    \"satoshis\"  (number) The difference of satoshis\n"
            "    \"txid\"  (string) The related txid\n"
            "    \"index\"  (number) The related input or output index\n"
            "    \"height\"  (number) The block height\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"tokenName\"  (string) The token associated with the balance (VBC for VibofChain)\n"
            "    \"balance\"  (string) The current balance in satoshis\n"
            "    \"received\"  (string) The total number of satoshis received (including change)\n"
            "  },...\n"
            "\n]"
    ...
    if (includeTokens) {
        ...
        for (std::vector<std::pair<uint160, int> >::iterator it = addresses.begin(); it != addresses.end(); it++) {
            if (!GetAddressIndex((*it).first, (*it).second, VBC, addressIndex)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No information available for address");
            }
        }
