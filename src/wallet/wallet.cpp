...
void CWallet::AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapTokenCoins, bool fGetVBC, bool fGetTokens, bool fOnlySafe, const CCoinControl *coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t& nMaximumCount, const int& nMinDepth, const int& nMaxDepth, bool fLockedTokens) const {
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetVBC) { // Looking for VBC Tx OutPoints Only
                    if (fVBCLimitHit) // We hit our limit
                        continue;

                    // We only want VBC OutPoints. Don't include Token OutPoints
                    if (isTokenScript)
                        continue;
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListTokens() const
{
    ...
    // https://github.com/VibofChain/VibofChain/pull/10340, but that change has been
    // postponed until after https://github.com/VibofChain/VibofChain/pull/10244 to
                                   ...
                                   const KnownTokenType& tokenType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-vibofchain-address
 ...
 * https://vibofchaintalk.org/?topic=54527, or
 * https://github.com/VibofChain/VibofChain/pull/1393.
 */
