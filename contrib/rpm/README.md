...
The RPM spec file provided here is for VibofChain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://vibofchain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[VibofChain git repository](https://github.com/VibofChain/VibofChain) but are not part of
...
Currently only `Source10` is used, for the example `vibofchain.conf` file.
...
in the vibofchain git repository. At present only an SVG version of the VibofChain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the vibofchain
...
needed when 0.12.1 is released, a proper fix is already in the VibofChain git
master branch.
...
idea to build VibofChain Core against a static version of that library compiled
...
The `vibofchain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `vibofchain-server` package. At this point, most RPM based
...
The `vibofchain-server` package should have SELinux support. How to properly do
...
VibofChain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/vibofchain/). He has been
packaging VibofChain for Fedora at least since 2012.
...
4. I renamed the `vibofchain` package that contains the Qt GUI to `vibofchain-core` as
that appears to be how the general population refers to it, in contrast to
`vibofchain-xt` or `vibofchain-classic`. I wanted to make sure the general population
