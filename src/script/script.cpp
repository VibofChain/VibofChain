...
int SearchForToken(const CScript& script, const int startingValue) {
    ...
    if (script[startingValue] == TOKEN_VBC) { // Check to see if RVN starts at the starting value ( this->size() < 105)
        if (script[startingValue + 1] == TOKEN_LOCAL)
            if (script[startingValue + 2] == TOKEN_PAYMENT)
                index = startingValue + 3;
    } else {
        if (script[startingValue + 1] == TOKEN_VBC) // Check to see if RVN starts at starting value + 1 ( this->size() >= 105)
