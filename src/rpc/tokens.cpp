...
UniValue issue(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreTokensDeployed() || request.params.size() < 1 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "4. \"change_address\"        (string), optional, default=\"\"), address the the VBC change will be sent to, if it is empty, change address will be generated for you\n"
    ...
    if (!changeAddress.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                               std::string("Invalid Change Address: Invalid VibofChain address: ") + changeAddress);
        }
    }
...
UniValue issueunique(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreTokensDeployed() || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "Hunderd (100) VBC will be burned for each token created.\n"
                ...
                "5. \"change_address\"        (string, optional, default=\"\"), address the the VBC change will be sent to, if it is empty, change address will be generated for you\n"
...
UniValue listtokenbalancesbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !AreTokensDeployed() || request.params.size() < 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"                  (string, required) a vibofchain address\n"
...
UniValue transferfromaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || !AreTokensDeployed() || request.params.size() < 4 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "\nArguments:\n"
                "1. \"address\"                  (string, required) valid VBC address\n"
