...
BOOST_FIXTURE_TEST_SUITE(token_tx_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(token_tx_not_valid_test)
    {
        ...
        // This should fail because 900 VIBOFCHAIN doesn't have a destination
        CTokenTransfer tokenTransfer("VIBOFCHAIN", 100, 0);
    ...
    BOOST_AUTO_TEST_CASE(token_tx_valid_multiple_outs_test)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            CTokenTransfer token2("VIBOFCHAIN", 100, 0);
    ...
    BOOST_AUTO_TEST_CASE(token_tx_multiple_tokens_test)
    {
        ...
        CTokenTransfer token2("VIBOFCHAINTEST", 1000, 0);
        CScript scriptPubKey2 = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        token2.ConstructTransaction(scriptPubKey2);

        CTokenTransfer token3("VIBOFCHAINTESTTEST", 1000, 0);
        ...
        {
            // Add the first token
            CTokenTransfer outToken("VIBOFCHAIN", 100, 0);
            ...
            CTokenTransfer outToken2("VIBOFCHAINTEST", 100, 0);
            ...
            // Add the third token
            CTokenTransfer outToken3("VIBOFCHAINTESTTEST", 100, 0);
        ...
        // The inputs are spending 3000 Tokens (1000 of each VIBOFCHAIN, VIBOFCHAINTEST, VIBOFCHAINTESTTEST)
        // The outputs are spending 100 Tokens to 10 destinations (10 * 100 = 1000) (of each VIBOFCHAIN, VIBOFCHAINTEST, VIBOFCHAINTESTTEST)
