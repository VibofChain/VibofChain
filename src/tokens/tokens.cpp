...
static const std::regex PROTECTED_NAMES("^VBC$|^VIBOFCHAIN$|^VBCCOIN$|^VBCHACOIN$|^VBCHACHAIN$");
...
void CNewToken::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(TOKEN_VBC);
...
{
    // Issuing an Token must contain at least 3 CTxOut( VibofChain Burn Tx, Any Number of other Outputs ..., Owner Token Tx, New Token Tx)
    if (vout.size() < 3) {
...
{
    // Must contain at least 3 outpoints (VBC burn, owner change and one or more new unique tokens that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
{
    // Reissuing an Token must contain at least 3 CTxOut ( VibofChain Burn Tx, Any Number of other Outputs ..., Reissue Token Tx, Owner Token Change Tx)
    if (vout.size() < 3) {
...
{
    // Check the first transaction and verify that the correct VBC Amount
...
{
    // Verify 'VBCq' is in the transaction
...
{
    // Verify 'VBCr' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
...
bool CreateTokenTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewToken> tokens, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string message)
{
    ...
    if (!change_address.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid VibofChain address: ") + change_address);
            return false;
        }
    } else {
    ...
    // Check to make sure the wallet has the VBC required by the burnAmount
...
bool CreateReissueTokenTransaction(CWallet* pwallet, CCoinControl& coinControl, const CReissueToken& reissueToken, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string message)
{
    ...
    if (!IsValidDestinationString(address)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid VibofChain address: ") + address);
        return false;
    }
...
bool CreateTransferTokenTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CTokenTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string message)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any VBC, transfering an token requires a network fee"));
        return false;
    }
