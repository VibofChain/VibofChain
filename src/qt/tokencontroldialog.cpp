    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/VibofChain/VibofChain/issues/5716
...
{
    GUIUtil::setClipboard(VibofChainUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void TokenControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = VibofChainUnits::VBC;
    ...
    l2->setText(VibofChainUnits::formatWithCustomName(QString::fromStdString(strTokenName), nTokenAmount));        // Amount
    l3->setText(VibofChainUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(VibofChainUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(VibofChainUnits::formatWithCustomName(QString::fromStdString(strTokenName), nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const std::pair<QString, std::vector<COutput>> &coins : mapTokenCoins) {
        ...
        for (const COutput &out : coins.second) {
            ...
            if (ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show vibofchain address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, VibofChainUnits::format(nDisplayUnit, nAmount));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, VibofChainUnits::format(nDisplayUnit, nSum));
