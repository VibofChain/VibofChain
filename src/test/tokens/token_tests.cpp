...
BOOST_FIXTURE_TEST_SUITE(token_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of VIBOFCHAINCOIN NOT allowed
        BOOST_CHECK(!IsTokenNameValid("VBC", type));
        BOOST_CHECK(!IsTokenNameValid("VIBOFCHAIN", type));
        BOOST_CHECK(!IsTokenNameValid("VIBOFCHAINCOIN", type));

        //- Versions of VIBOFCHAINCOIN ALLOWED
        BOOST_CHECK(IsTokenNameValid("VIBOFCHAIN.COIN", type));
        BOOST_CHECK(IsTokenNameValid("VIBOFCHAIN_COIN", type));
        BOOST_CHECK(IsTokenNameValid("VBCSPYDER", type));
        BOOST_CHECK(IsTokenNameValid("SPYDERVBC", type));
        BOOST_CHECK(IsTokenNameValid("VIBOFCHAINSPYDER", type));
        BOOST_CHECK(IsTokenNameValid("SPYDEVIBOFCHAIN", type));
        BOOST_CHECK(IsTokenNameValid("BLACK_VIBOFCHAINS", type));
        BOOST_CHECK(IsTokenNameValid("SEVBCOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_token_coin_test)
    {
        ...
        CTokenTransfer token("VIBOFCHAIN", 1000, 0);
    ...
    BOOST_AUTO_TEST_CASE(new_token_coin_test)
    {
        ...
        // Create the token scriptPubKey
        CNewToken token("VIBOFCHAIN", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        token.ConstructTransaction(scriptPubKey);
