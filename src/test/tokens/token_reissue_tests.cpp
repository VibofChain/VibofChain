...
#include <test/test_vibofchain.h>
...
BOOST_FIXTURE_TEST_SUITE(token_reissue_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(reissue_cache_test)
    {
        ...
        CNewToken token1("VBCTOKEN", CAmount(100 * COIN), 8, 1, 0, "");

        // Add an token to a valid VBC address
        ...
        CReissueToken reissue1("VBCTOKEN", CAmount(1 * COIN), 8);
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedTokenData.count("VBCTOKEN"), "Map Reissued Token should contain the token \"VBCTOKEN\"");
        BOOST_CHECK_MESSAGE(cache.mapTokensAddressAmount.at(make_pair("VBCTOKEN", Params().GlobalBurnAddress())) == CAmount(101 * COIN), "Reissued amount wasn't added to the previous total");

        // Get the new token data from the cache
        CNewToken token2;
        BOOST_CHECK_MESSAGE(cache.GetTokenMetaDataIfExists("VBCTOKEN", token2), "Failed to get the token2");
        ...
        BOOST_CHECK_MESSAGE(token2.strName == "VBCTOKEN", "Token2: Token name is wrong");
        ...
        undoBlockData.emplace_back(std::make_pair("VBCTOKEN", CBlockTokenUndo{true, false, "", 0}));
        ...
        CNewToken token3;
        BOOST_CHECK_MESSAGE(cache.GetTokenMetaDataIfExists("VBCTOKEN", token3), "Failed to get the token3");
        ...
        BOOST_CHECK_MESSAGE(token3.strName == "VBCTOKEN", "Token3: Token name is wrong");
        BOOST_CHECK_MESSAGE(token3.units == 8, "Token3: Units is wrong");

        // Check to see if the reissue removal updated the cache correctly
        BOOST_CHECK_MESSAGE(cache.mapReissuedTokenData.count("VBCTOKEN"), "Map of reissued data was removed, even though changes were made and not databased yet");
        BOOST_CHECK_MESSAGE(cache.mapTokensAddressAmount.at(make_pair("VBCTOKEN", Params().GlobalBurnAddress())) == CAmount(100 * COIN), "Tokens total wasn't undone when reissuance was");
    }
    ...
    BOOST_AUTO_TEST_CASE(reissue_isvalid_test)
    {
        ...
        CReissueToken reissue3("VBCTOKEN", CAmount(1 * COIN), 7);
        ...
        CReissueToken reissue4("VBCTOKEN", CAmount(1 * COIN), -1);

        BOOST_CHECK_MESSAGE(reissue4.IsValid(error, cache), "Reissue4 wasn't valid");

        // Create a new token object with units of 0
        CNewToken token2("VBCTOKEN2", CAmount(100 * COIN), 0, 1, 0, "");

        // Add new token to a valid VBC address
        ...
        CReissueToken reissue5("VBCTOKEN2", CAmount(1 * COIN), 1);
        ...
        CReissueToken reissue6("VBCTOKEN2", CAmount(1 * COIN), 1);
