...
public:
    explicit TxViewDelegate(const PlatformStyle *_platformStyle, QObject *parent=nullptr):
        QAbstractItemDelegate(parent), unit(VibofChainUnits::VBC),
    ...
    tokendelegate(new TokenViewDelegate(platformStyle, this))
{
    ...
    ui->VBCBalancesLabel->setStyleSheet(STRING_LABEL_COLOR);
    ...
    ui->VBCBalancesLabel->setFont(GUIUtil::getTopLabelFont());
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& stake, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance, const CAmount& watchOnlyStake, const CAmount& lockedBalance)
{
    ...
    ui->labelBalance->setText(VibofChainUnits::formatWithUnit(unit, balance, false, VibofChainUnits::separatorAlways));
    ui->labelUnconfirmed->setText(VibofChainUnits::formatWithUnit(unit, unconfirmedBalance + lockedBalance, false, VibofChainUnits::separatorAlways));
    ui->labelImmature->setText(VibofChainUnits::formatWithUnit(unit, immatureBalance, false, VibofChainUnits::separatorAlways));
    ui->labelStake->setText(VibofChainUnits::formatWithUnit(unit, stake, false, VibofChainUnits::separatorAlways));
    ui->labelTotal->setText(VibofChainUnits::formatWithUnit(unit, balance + unconfirmedBalance + immatureBalance + lockedBalance, false, VibofChainUnits::separatorAlways));
    ui->labelWatchAvailable->setText(VibofChainUnits::formatWithUnit(unit, watchOnlyBalance, false, VibofChainUnits::separatorAlways));
    ui->labelWatchPending->setText(VibofChainUnits::formatWithUnit(unit, watchUnconfBalance, false, VibofChainUnits::separatorAlways));
    ui->labelWatchImmature->setText(VibofChainUnits::formatWithUnit(unit, watchImmatureBalance, false, VibofChainUnits::separatorAlways));
    ui->labelWatchTotal->setText(VibofChainUnits::formatWithUnit(unit, watchOnlyBalance + watchUnconfBalance + watchImmatureBalance + watchOnlyStake, false, VibofChainUnits::separatorAlways));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("VBC")
    updateDisplayUnit();
}
...
void OverviewPage::showTokens()
{
    if (AreTokensDeployed()) {
        ...
        // This keeps the VBC balance grid from expanding and looking terrible when token balance is hidden
